...
def apply_primitive(procedure, args, env):
    ...
    try:
        ...
        raise SchemeError("line93 apply Primitive")
...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) == len(vals):
            ...
            raise SchemeError("line 148 make call frame")
        return frame
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        #     raise SchemeError("improper name")
        # env.define(target.first,do_lambda_form(Pair(formals,vals.second),env))
        # return target.first
...
def do_if_form(vals, env):
    ...
    elif vals.second.second is nil:
        return okay
    return vals[2]
...
def check_formals(formals):
    ...
    if not scheme_listp(formals):
        raise SchemeError("formals are not lists")
    a = []
    b = formals
    while b is not nil:
        if not scheme_symbolp(b.first):
            raise SchemeError("formals are not symbols")
        a.append(b.first)
        b = b.second
    if len(a) != len(set(a)):
        raise SchemeError("formals contain repeated symbols")
##################
# Tail Recursion #
##################
