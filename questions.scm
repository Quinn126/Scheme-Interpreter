...
(define (list-partitions total max-pieces max-value)
  (define (helper sum pieces max sofar)
          (cond 
            ((= sum 0) (list sofar))
            ((< sum 0) '())
            ((= max 0) '())
            ((= pieces 0) '())
            (else (append
                  (helper sum pieces (- max 1) sofar)
                  (helper (- sum max) (- pieces 1) max (append sofar (list max)))))))
...
(define (tree-sums tree)
  (if (null? (children tree)) (list (entry tree))
      (begin (define children-sum (map tree-sums (children tree)))
       (define (add child) (+ child (entry tree)))
       (define flattened (flatten children-sum))
       (map add flattened))))
;(tree-sums tree) 
